...
Task_rho = 5000; % Task的计算密度(cycles*MB)
...
%% Initialization
% 首先生成一个初始的A
A_global = zeros(M,N);  % AP clustering matrix
A_local = zeros(M,N); % Local copy of clustering matirx
...
C_global = zeros(M,N);  % Computing resource allocation matrix
B_global = zeros(M,N);  % Bandwidth resource allocation matrix
...
% while norm(A_global - A_local)>= theta && t<=t_max
while t<=t_max
...
for i=1:M  % for each AP, solve a optimization sub-problem
...
Total_energy = V
L;
Penalty-DC_penalty_C_re + DC_penalty_B_re;

...
MC_OO_list = zeros(1,5);
MC_NO_list = zeros(1,5);
MC_RO_list = zeros(1,5);
MC_proposed_list = zeros(1,5);
% consensus_energy
MC_OO_list_con = zeros(1,5);
MC_NO_list_con = zeros(1,5);
MC_RO_list_con = zeros(1,5);
MC_proposed_list_con = zeros(1,5);
...
for index = 1:1:5
    B_m = randi([B_value(index),B_value(index)],1,M);   % AP m的带宽资源(MHz)
% Initilazation
%% Parameters Setting
...
IP_offloading_energy_list = zeros(4,5) % OO,NO,RO,proposed
IP_consensus_energy_list = zeros(4,5)
